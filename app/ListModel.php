        ...
        if ($pivot->isNotEmpty()) {
            $items = $pivot
                ...
                ->map(function(Collection $records, $model) {
                    ...
                    $builder = app($model)
                        ->whereIn('id', $records->pluck('listable_id'));

                    if (app(Settings::class)->get('streaming.show_label') && $model === Title::class) {
                        $builder->withCount('stream_videos');
                    }

                    $items = $builder->get($select);

                    if ($model === Title::class) {
                        $preferFullVideos = app(Settings::class)->get('streaming.prefer_full');
                        $items->load(['videos' => function(HasMany $query) use($items, $preferFullVideos) {
