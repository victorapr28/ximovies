...
class Title extends Model
{
    ...
    protected $appends = ['rating', 'type', 'vote_count'];
    ...
    {
        $column = str_replace('_average', '_count', config('common.site.rating_column'));
    ...
    public function getLastAndNextEpisodes()
    {
        $episodes = app(Episode::class)
            ...
            ->whereBetween('release_date', [Carbon::now()->subWeek(2), Carbon::now()->addWeek(2)])
        ...
        if ($episodes->count() < 2 || ( !$episodes[0]['poster'] && !$episodes[1]['poster'])) {
            return null;
        }
