...
class TransformData
{
    ...
    private function getVideos($tmdbTitle, $year, $titleName)
    {
        ...
        return array_map(function($video) use($titleName, $year) {
            // remove title name from "title name (year) - video name"
            $titleName = preg_quote($titleName, '/');
            $videoName = preg_replace("/$titleName/i", '', $video['name']);
            $videoName = preg_replace("/\($year\)/i", '', $videoName);
            $videoName = trim($videoName, " \t\n\r \v-\"|–");
            $videoName = trim(preg_replace('!\s+!', ' ', $videoName));
            return [
                'name' => trim($videoName),
