...
class GetPersonCredits
{
    ...
    public function execute(Person $person, $options = [])
    {
        ...
        $mergedCredits = $this->mergeCredits(Arr::get($credits, 'all', []), $seasonCredits, $episodeCredits);
    ...
    private function mergeCredits($credits1, $credits2, $credits3)
    {
        ...
        $mergedCredits = array_map(function($titles) {
            ...
            foreach ($titles as $title) {
                ...
                if ($existing) {
                    $existing['credited_episode_count'] = Arr::get($existing, 'credited_episode_count', 0) +  Arr::get($title, 'credited_episode_count', 0);
                    $existing['episodes'] = collect(array_merge(Arr::get($existing, 'episodes', []), Arr::get($title, 'episodes', [])))->unique('id')->toArray();
                    if ( ! $this->titleId) {
                        $existing['episodes'] = array_slice($existing['episodes'], 0, 5);
                    }
    ...
    private function getTitleCredits(Person $person)
    {
        $credits = $person
            ->credits()
            ->withCount(app(Settings::class)->get('streaming.show_label') ? 'stream_videos' : null)
            ->get();
    ...
    {
        $seasons = $person->seasonCredits($this->titleId)->with([
            'title' => function($query) {
                return $query->select('id', 'name', 'year', 'poster');
            },
            'episodes' => function($query) {
                return $query->select('id', 'name', 'year', 'season_id', 'season_number', 'episode_number', 'title_id', 'release_date')
        ...
        return $groupedSeasons->map(function (Collection $departmentGroup) {
            ...
            return $seasonsGroupedByTitle->map(function (Collection $titleSeasons) {
                ...
                $episodesFromAllSeasons = $titleSeasons
                    ...
                    ->map(function($episode) use($titleSeasons) {
                        $episode->pivot = $titleSeasons->first()->pivot->toArray();
                        return $episode;
                    });
                $title['credited_episode_count'] = $episodesFromAllSeasons->count();
                if ( ! $this->titleId) {
                    $episodesFromAllSeasons = $episodesFromAllSeasons->take(5);
                }
